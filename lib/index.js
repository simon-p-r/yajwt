...
internals.schema = {
    sign: Joi.object().keys({
        ...
        payload: Joi.object().keys({
            ...
            iat: Joi.alternatives().try(Joi.number(), Joi.string()).default(Helpers.msToSeconds(Date.now())),
            iss: Joi.string(),
            jti: Joi.string(),
...
exports.sign = (options, cb) => {
    ...
    Joi.validate(options, schema, (err, result) => {
        ...
        result.payload = Helpers.coercePayload(result.payload);
...
exports.signSync = (options) => {
    ...
    validOptions.value.payload = Helpers.coercePayload(validOptions.value.payload);
